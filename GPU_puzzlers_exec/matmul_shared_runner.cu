...
extern __global__ void matmul_shared(float *A, float *B, float *C, int M, int N, int K, int BLOCK_M, int BLOCK_N, int BLOCK_K);
...
void run_kernel(){
  ...
  int shared_size = (BLOCK_M*BLOCK_K + BLOCK_K*BLOCK_N) * sizeof(float);
  cudaEventRecord(start);
  matmul_shared<<<grid_dim, block_dim, shared_size>>>(A, B, C, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K);
  cudaEventRecord(end);
  ...
  if (!fail) printf("shared matmul kernel took %f ms\n", ms);
  cudaFree(A);
  cudaFree(B);
  cudaFree(C);
}
