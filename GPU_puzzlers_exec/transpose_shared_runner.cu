...
extern __global__ void _transpose_shared(float *in, float *out, int M, int N);
...
void runKernel(){
  ...
  dim3 block_dim(BLOCK_N, BLOCK_M, 1);
  const int shared_size = BLOCK_M*BLOCK_N*sizeof(float);
  _transpose_shared<<<grid_dim, block_dim, shared_size>>>(in, out, M, N);
